...
-- of truth the "87% win rate" style stats get computed from, so
-- there is nothing to fabricate: the number is just a query.
-- ─────────────────────────────────────────────────────────────
...
-- ever seeded, so every message a subscriber sees came from a
-- real person.
-- ─────────────────────────────────────────────────────────────
...
-- Paystack webhook handling all happen in application code, not
-- in Postgres policies). RLS here is a deliberately strict safety
-- net: it assumes direct table access from the browser's anon key
-- and only allows the minimum a signed-in user needs.
-- ─────────────────────────────────────────────────────────────
...
-- feature exists in the app — profiles are only ever written by
-- the signup trigger above or by Route Handlers using the service
-- role (Paystack activation, admin actions). If you add a "change
-- my name" feature later, give it its own policy that explicitly
