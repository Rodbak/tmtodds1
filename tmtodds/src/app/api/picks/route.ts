...
export async function GET(request: NextRequest) {
  ...
  if (scope === "ledger") {
    const [{ data: settled, error }, { count }] = await Promise.all([
      supabase
        .from("picks")
        .select("*")
        .neq("status", "pending")
    ...
    const rows = (settled ?? []) as PickRow[];
    const items: PickDTO[] = rows.map((p) => ({
      id: p.id,
      league: p.league,
      fixture: p.fixture,
      kickoffAt: p.kickoff_at,
      tier: p.tier as PickDTO["tier"],
      status: p.status,
    ...
    const stats = computeStats(rows, count ?? rows.length);
